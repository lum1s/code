...
while True:
    ...
    try:
        ...
        if text.find(':!off') !=-1 and (textTT[0] == 'prezesS' or textTT[1].find('prezesS') !=-1 or textTT[0] == 'lum1s' or textTT[1].find('lum1s')!=-1):
            fun = mode(irc,"sleep",channel)
            print 'fin mode is set: %s' %fun

        if text.find(':!on') !=-1 and (textTT[0] == 'prezesS' or textTT[1].find('prezesS') !=-1 or textTT[0] == 'lum1s' or textTT[1].find('lum1s')!=-1):
        ...
        if text2.find('PRIVMSG luBot') ==-1:
            ...
            if fun == 1:
                ...
                if text.find(':!info') !=-1 and (textTT[0] == 'prezesS' or textTT[1].find('prezesS') !=-1 or textTT[0] == 'lum1s' or textTT[1].find('lum1s')!=-1):
            ...
            if (',add?' in text) and ('lum1s' in text):
                addDB(irc,dataBase,text)
            if (':!db_new' in text) and ('lum1s' in text):
                createDB(irc,dataBase)
            if (':!db_show' in text) and ('lum1s' in text):
                showDB(irc,dataBase)
            if (':!join' in text) and ('lum1s' in text):
                ch = text.split('!join')
                print 'ch: %s' %ch[1]
                join_channel(irc,ch[1])
            if (':!db_find' in text) and ('lum1s' in text):
                tmp = text.split(':!db_find')
                searchDB(irc,dataBase,tmp[1].strip())
